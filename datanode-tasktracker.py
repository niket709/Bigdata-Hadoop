...
import   commands,time,ast,numpy
...
for dn_ip in datanodes:
	commands.getoutput("sshpass -p 'q' scp /root/Desktop/core_conf_file root@"+dn_ip+":/etc/hadoop/")

	commands.getoutput("sshpass -p 'q' scp /root/Desktop/mapred_conf_file root@"+dn_ip+":/etc/hadoop/")

	commands.getoutput("sshpass -p 'q' scp /root/Desktop/dhdfs_conf_file root@"+dn_ip+":/etc/hadoop/")

	t1="sshpass -p 'q' ssh -o StrictHostKeyChecking=no root@"+dn_ip+" "+'\'sed -i "/<configuration>/r /etc/hadoop/mapred_conf_file" /etc/hadoop/mapred-site.xml\''

	t2="sshpass -p 'q' ssh -o StrictHostKeyChecking=no root@"+dn_ip+" "+'\'sed -i "/<configuration>/r /etc/hadoop/core_conf_file" /etc/hadoop/core-site.xml\''

	t3="sshpass -p 'q' ssh -o StrictHostKeyChecking=no root@"+dn_ip+" "+'\'sed -i "/<configuration>/r /etc/hadoop/dhdfs_conf_file" /etc/hadoop/hdfs-site.xml\''

		
	commands.getoutput(t1)

	commands.getoutput(t2)
